...
namespace DotNet.HalconUI
{
    ...
    /// 通过左键拖动以圆形画笔擦除区域
    /// </summary>
    public class EditEraseRectHandler : IDrawHandler
    {
        ...
        private static void EraseAt(DisplayUI display, HTuple row, HTuple column)
        {
            DrawCircle(display, row, column);
            DispEraseRegion(display);
        }

        private static void DrawCircle(DisplayUI display, HTuple row, HTuple column)
        {
        ...
        private static void DispEraseRegion(DisplayUI display)
        {
