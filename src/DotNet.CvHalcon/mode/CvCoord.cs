...
namespace DotNet.CvHalcon
{
    ...
    public readonly record struct CvCoord : IEquatable<CvCoord>, ICvTranslatable<CvCoord>, ICvRotatable<CvCoord>
    {
        ...
        /// <param name="angle">角度（弧度）</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public CvCoord(CvPoint center, double angle = 0)
        {
            X = center.X;
            Y = center.Y;
            Angle = NormalizeAngle(angle);
        ...
        /// 绕指定点旋转
        /// </summary>
        public CvCoord RotateAround(double deltaAngle, CvPoint pivot)
        {
            // 先旋转位置
            CvPoint newCenter = Center.RotateAround(deltaAngle, pivot);
            // 再更新角度
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public CvCoord Translate(CvPoint offset)
        {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public CvPoint WorldToLocal(CvPoint worldPoint)
        {
            double dx = worldPoint.X - X;
            double dy = worldPoint.Y - Y;
            double cos = Math.Cos(-Angle);
            double sin = Math.Sin(-Angle);
            return new CvPoint(dx * cos - dy * sin, dx * sin + dy * cos);
        }
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public CvPoint LocalToWorld(CvPoint localPoint)
        {
            double cos = Math.Cos(Angle);
            double sin = Math.Sin(Angle);
            return new CvPoint(
        ...
        {
            CvPoint newCenter = LocalToWorld(other.Center);
